...
def run_all_four_visualizations():
    ...
    for ax, (name, ptype, energy, substr, cmap_name, c_line, c_dark) in zip(axs1, configs):
        ...
        if len(dep) > 0:
            ...
            ax.pcolormesh(X, Z, H_smooth.T, norm=mcolors.LogNorm(vmin=H_smooth.max()*1e-4, vmax=H_smooth.max()),
    ...
    plt.savefig(os.path.join(LEGACY_DIR, 'master_fig1_interaction_volume.png'))
    ...
    plt.savefig(os.path.join(LEGACY_DIR, 'master_fig2_psf_nils.png'))
    ...
    plt.savefig(os.path.join(LEGACY_DIR, 'master_fig3_ler_statistics.png'))
    ...
    plt.savefig(os.path.join(LEGACY_DIR, 'master_fig4_process_window.png'))
